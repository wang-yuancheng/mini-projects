...
class OilSpillCNN(nn.Module):
    def __init__(self, in_bands, embed_dim=128, num_classes=1):
        super().__init__()
        # embed_dim is maintained in the signature for MoCo compatibility but ignored for the 2D CNN
...
# ==========================================
PRETRAIN_EPOCHS = 40
# Renamed to avoid overwriting your old transformer weights
PRETRAINED_WEIGHTS_PATH = f"moco_pretrained_cnn_{len(CLEAN_BANDS)}.pth"
...
# Target index 4 because self.classifier in OilSpillCNN starts with Flatten() at index 0
model.classifier[4] = nn.Linear(64, 1).to(device)

criterion = nn.BCEWithLogitsLoss() 
optimizer = torch.optim.AdamW(model.parameters(), lr=3e-4, weight_decay=1e-3)
FINETUNE_EPOCHS = 30
FINETUNED_WEIGHTS_PATH = f"finetuned_cnn_{len(CLEAN_BANDS)}.pth"
